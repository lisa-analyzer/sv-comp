...
def statistics():
        ...
        if (os.path.isdir(res_dir)):
            ...
            for file in os.listdir(res_dir):
                if (file == "frontend.csv"):
                    temp = pandas.read_csv(os.path.join(res_dir, file), sep=";")[["Message", "Type"]].groupby(
                        ["Message"]).count()
    ...
    summary_lines = [
        f"Number of test cases: [bold blue]{test_cases}[/bold blue]",
        f"Score: [bold green]{dataframe_score['Score'].sum()}[/bold green] \n",
        f"Parsing errors (parsing.csv): [bold red]{error_parsing}[/bold red]",
        f"Frontend errors (frontend.csv): [bold red]{error_frontend}[/bold red]",
        f"Analysis errors (analysis.csv): [bold red]{error_analysis}[/bold red]"
    ]

    for line in summary_lines:
        rich.print(line)

    with open(f"{config.path_to_output_dir}/summary.txt", "w") as f:
        for line in summary_lines:
            plain_line = Text.from_markup(line).plain
            f.write(plain_line + "\n")
